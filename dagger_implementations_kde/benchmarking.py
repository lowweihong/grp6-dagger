...
Batch mode:
- --run-batch runs a sweep over N_DAGGER_ITERATIONS in {10,15,20,25,30,35,40}
  * dagger_mod_{N}   : KDE ON, action noise ON
  * dagger_vanilla_{N}: KDE OFF, action noise OFF
"""
...
N_DAGGER_ITERATIONS = 35      # Default number of DAgger iterations (overridden by --iterations)
...
def run_dagger_experiment(args):
    """
    Run a single DAgger experiment using the provided argparse.Namespace `args`.

    `args.experiment_name` controls base filenames:
      - results/{experiment_name}.keras
      - results/{experiment_name}_performance.txt
      - results/{experiment_name}_results.json
      - TensorBoard: {tensorboard_log}/{experiment_name}/iteration_i/...
    """
    experiment_name = getattr(args, "experiment_name", None) or "dagger_experiment"

    # If user didn't override student-model, tie it to experiment name
    if args.student_model == STUDENT_MODEL_FILE:
        args.student_model = f"{experiment_name}.keras"
    ...
    if not args.no_tensorboard:
        # Per-experiment TensorBoard root
        experiment_log_root = os.path.join(args.tensorboard_log, experiment_name)
        os.makedirs(experiment_log_root, exist_ok=True)
        ...
        print(f"TensorBoard logs will be saved to: {experiment_log_root}")
    ...
    if not args.no_tensorboard:
        summary_writer = tf.summary.create_file_writer(experiment_log_root)
    else:
        summary_writer = None
    ...
    for i in range(args.iterations):
        ...
        if tensorboard_callback:
            ...
            iter_log_dir = os.path.join(experiment_log_root, f"iteration_{i+1}")
            tensorboard_callback.log_dir = iter_log_dir
            callbacks.append(tensorboard_callback)
        
        # Add checkpoint callback to save model after each iteration
        os.makedirs("results", exist_ok=True)
        checkpoint_path = os.path.join("results", f"{experiment_name}_checkpoint_iter_{i+1}.keras")
        ...
        if i < 3:  # Only for first few iterations
            ...
            for j in range(min(3, len(test_states))):
    ...
    print("\n" + "=" * 60)
    print(f"Modified DAgger Training Complete for experiment: {experiment_name}")
    print("=" * 60)
    ...
    os.makedirs("results", exist_ok=True)
    # If student_model path already includes directory, use it as-is, otherwise add results/
    if os.path.dirname(args.student_model):
    ...
    # Save performance log (text)
    log_file = os.path.join("results", f"{experiment_name}_performance.txt")
    with open(log_file, 'w') as f:
        f.write(f"Modified DAgger Training Results for experiment: {experiment_name}\n")
        f.write(f"KDE Enabled: {not args.no_kde_filtering}\n")
        f.write(f"Action Noise Prob: {args.action_noise_prob}\n")
    ...
    # Save JSON results
    json_file = os.path.join("results", f"{experiment_name}_results.json")
    results_data = {
        "training_info": {
            ...
            "kde_enabled": not args.no_kde_filtering
        },
        "expert_performance": {
    ...
    if summary_writer is not None:
        ...
        print(f"\nTensorBoard logs saved to: {experiment_log_root}")
        print(f"To view: tensorboard --logdir {args.tensorboard_log}")
...
def main():
    ...
    # NEW: experiment naming and batch mode
    parser.add_argument('--experiment-name', type=str, default=None,
                        help='Base name for output files (model/logs). If not set, a default based on iterations is used.')
    parser.add_argument('--run-batch', action='store_true',
                        help='Run a sweep over iterations with KDE+noise and vanilla (KDE OFF, noise OFF)')
    
    args = parser.parse_args()

    if args.run_batch:
        # Predefined sweep
        sweep_iters = [10, 15, 20, 25, 30, 35, 40]

        for n_iter in sweep_iters:
            # --- Modified variant: KDE ON, action noise ON ---
            mod_args = argparse.Namespace(**vars(args))
            mod_args.iterations = n_iter
            mod_args.no_kde_filtering = False
            # Ensure noise ON
            if mod_args.action_noise_prob <= 0.0:
                mod_args.action_noise_prob = ACTION_NOISE_PROB
            mod_args.experiment_name = f"dagger_mod_{n_iter}"

            print(f"\n\n########## Running experiment {mod_args.experiment_name} ##########")
            run_dagger_experiment(mod_args)

            # --- Vanilla variant: KDE OFF, action noise OFF ---
            vanilla_args = argparse.Namespace(**vars(args))
            vanilla_args.iterations = n_iter
            vanilla_args.no_kde_filtering = True
            vanilla_args.action_noise_prob = 0.0
            vanilla_args.experiment_name = f"dagger_vanilla_{n_iter}"

            print(f"\n\n########## Running experiment {vanilla_args.experiment_name} ##########")
            run_dagger_experiment(vanilla_args)
    else:
        # Single experiment mode (original behavior, just with experiment naming)
        if args.experiment_name is None:
            args.experiment_name = f"dagger_single_{args.iterations}"
        run_dagger_experiment(args)
