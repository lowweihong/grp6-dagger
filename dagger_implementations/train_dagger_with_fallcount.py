...
# --- REMOVED postprocess_student_action ---
# (Activations are now in the model)
...
def main():
    ...
    parser.add_argument('--tensorboard-log', type=str, default='./dagger_tensorboard2',
    ...
    for i in range(args.iterations):
        ...
        # --- Step A: Collect Data (Rollout with Expert Mixing) ---
        print(f"Rolling out {args.episodes_per_iter} episode(s) with DAgger + Expert mixing...")
    ...
    # Save JSON results
    json_file = f"dagger_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    results_data = {
        "training_info": {
